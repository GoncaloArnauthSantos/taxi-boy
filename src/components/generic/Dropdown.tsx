...
export default function Dropdown<T>({ list, value, defaultValue, handleChange, mapId, mapValue }: Props<T>) {
  ...
  const dropdownHandleChange = (newValue: T) => {
    const newValueId = mapDropdownId(newValue)
    const currentValueId = value ? mapDropdownId(value) : null

    handleChange(newValueId === currentValueId ? null : newValue)
  }
  ...
  return (
    <div className="w-64">
      <Listbox value={value} onChange={dropdownHandleChange}>
        {({ open }) => (
          <div className="relative mt-1">
            ...
            >
              <span className="block truncate">{value ? mapDropdownValue(value) : defaultValue || "Select option"}</span>
